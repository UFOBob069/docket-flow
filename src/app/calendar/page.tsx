...
import { eventKindDisplayLabel } from "@/lib/one-off-events";
import type { CalendarEvent, Contact, EventCategory } from "@/lib/types";
...
export default function CalendarPage() {
  ...
  const rangeEnd = format(addDays(parseISO(rangeStart), weeksEndOffsetDays - 1), "yyyy-MM-dd");
  ...
  const filteredSorted = useMemo(() => {
    ...
    const list = rows.filter(({ event: e, case: c }) => {
      ...
      if (!e.included || e.noiseFlag) return false;
    ...
    return list;
  }, [rows, search, attorneyFilter, paralegalFilter]);
  ...
  return (
    <PageWrapper>
      ...
      {!refreshing && filteredSorted.length === 0 && !loadError && (
        <div className="mt-8">
          <EmptyState
            ...
            description="Adjust filters, load more weeks, or add deadlines from a case."
            action={
      ...
      <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-border pt-6">
        <p className="text-sm text-text-muted">
          {filteredSorted.length} event{filteredSorted.length !== 1 ? "s" : ""} match your filters in this
          window.
        </p>
        <Button
          variant="secondary"
