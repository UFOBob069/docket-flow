...
export async function executePromoteIntakeToCase(
  ...
  const caseId = await createCase(supabase, user.id, {
    ...
    clientEmail: body.clientEmail?.trim() || intake.email?.trim() || null,
    clientStreetAddress: body.clientStreetAddress?.trim() || intake.address?.trim() || null,
    clientCity: body.clientCity?.trim() || null,
    clientState: body.clientState?.trim() || null,
    clientZip: body.clientZip?.trim() || null,
    clientCountry: body.clientCountry?.trim() || null,
    caseNumber: cn,
    causeNumber: cn,
